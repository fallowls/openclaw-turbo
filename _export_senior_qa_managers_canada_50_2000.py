...
OUT = os.path.join(os.path.expanduser('~'), 'Downloads', 'canada_senior_qa_managers_50_2000.csv')
...
where employee_count between 50 and 2000
  ...
  and (
    -- senior
    title ~* '(^|\\m)(Senior|Sr\\.?)(\\M|$)'
    or title ~* 'Senior\\s+Manager'
    or title ~* 'Senior\\s+Quality'
  )
  and (
    -- manager
    title ~* '(^|\\m)(Manager|Mgr\\.?)(\\M|$)'
    or seniority ilike '%manager%'
  )
